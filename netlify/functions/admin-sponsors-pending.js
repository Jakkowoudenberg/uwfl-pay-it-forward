exports.handler = async function(event, context) {
  ...
  try {
    const select = ['id','company','country','logo_url','why','what','contact_website','contact_email','contact_phone','created_at'].join(',');
    const response = await fetch(`${process.env.SUPABASE_URL}/rest/v1/sponsors?select=${select}&status=eq.pending&order=created_at.asc`,
