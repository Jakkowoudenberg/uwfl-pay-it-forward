exports.handler = async function(event, context) {
  ...
  try {
    ...
    const uniqueName = `org_${Date.now()}_${Math.random().toString(36).slice(2, 8)}.${ext}`;
    const uploadResponse = await fetch(`${SUPABASE_URL}/storage/v1/object/org-logos/${uniqueName}`, {
      method: 'POST',
    ...
    const publicUrl = `${SUPABASE_URL}/storage/v1/object/public/org-logos/${uniqueName}`;
