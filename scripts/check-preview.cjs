...
const code=['content.js','translations.js','experience.js','purpose.js','community.js','shipping-policy.js','regional.js','app.js'].map(n=>fs.readFileSync(path.join(root,'assets',n),'utf8'));
const fixtures={
...
(async()=>{
 let routeChecks=0;
 const routes=['home','makers','visitors','help','about','partners','sponsor','organisations','organisation','media-partner','community','community?group=makers&country=NL','community?group=contributors','community?group=media','gallery','read/project','read/press','read/initiator','read/privacy','read/expo','read/kickoff','read/news','read/kunstwerk','read/regels','resources','drawing','panel','idea','join','join/maker','join/participant','join/contributor','join/student','upload','contact','contact/support','contact/organisation-support','contact/media'];
 ...
 const p=await create();await nav(p,'upload');p.d.querySelector('[data-action="sample-upload"]').click();for(const [id,value] of [['artwork-name','Fictief testpaneel'],['wood-species','Eiken'],['panel-story','Dit is een offline test, geen echte inzending.'],['shipping-country','NL']])fill(p,id,value);submit(p);assert(p.d.getElementById('form-error').textContent,'Missing panel photo accepted');selectImage(p,'panel-photos');for(const [country,region] of [['NL','eu'],['CA','americas'],['GB','other'],['NL','eu']]){const select=p.d.getElementById('shipping-country');select.value=country;select.dispatchEvent(new p.w.Event('change',{bubbles:true}));assert.equal(p.d.querySelector('[data-shipping-region]').dataset.shippingRegion,region);assert.equal(p.d.getElementById('artwork-name').value,'Fictief testpaneel');}submit(p);assert(p.d.querySelector('.review-list').textContent.includes('Nederland'));assert.equal(p.d.querySelector('[data-shipping-region]').dataset.shippingRegion,'eu');submit(p);assert(p.d.querySelector('.success-panel .moderation-note'),'Panel lacks moderation explanation');ensureReadOnly(p);p.close();
