...
const code=['content.js','translations.js','experience.js','purpose.js','community.js','app.js'].map(n=>fs.readFileSync(path.join(root,'assets',n),'utf8'));
const fixtures={
...
(async()=>{
 let routeChecks=0;
 const routes=['home','makers','visitors','help','about','partners','sponsor','organisations','organisation','media-partner','community','community?group=makers&country=NL','community?group=contributors','community?group=media','gallery','read/project','read/press','read/initiator','read/privacy','read/expo','read/regels','resources','drawing','panel','idea','join','join/maker','join/participant','join/contributor','join/student','upload','contact','contact/support','contact/organisation-support','contact/media'];
